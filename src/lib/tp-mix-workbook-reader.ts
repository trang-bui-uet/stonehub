...
type ReadTpMixWorkbookResult = Readonly<{
  supplierName: string
  sheetName: string
  sections: readonly ReadSupplierWorkbookSectionResult[]
}>
type ReadTpMixWorkbookParams = Readonly<{
  file: File
  supplierName: string
}>
...
function parseRowsByYellowMergedHeader(
  ...
  for (let rowNumber = firstRowNumber; rowNumber <= lastRowNumber; rowNumber += 1) {
    if (isMergedHeaderRow(sheet, rowNumber)) {
      const headerMaterialName = getColumnValueAsString(sheet, "A", rowNumber)
      const currentSection = parsedSections[sectionIndex]
      const canMoveToNextSection = sectionIndex + 1 < DEFAULT_MAX_MIX_SECTION
      if (currentSection.rows.length > 0 && canMoveToNextSection) {
        sectionIndex += 1
        parsedSections[sectionIndex].materialName = headerMaterialName
        continue
      }
      if (!currentSection.materialName) {
        currentSection.materialName = headerMaterialName
      }
      continue
    }
...
/**
 * Read TP mix workbook and split sections by yellow merged headers.
 */
export async function readTpMixWorkbookFromFile(params: ReadTpMixWorkbookParams): Promise<ReadTpMixWorkbookResult> {
